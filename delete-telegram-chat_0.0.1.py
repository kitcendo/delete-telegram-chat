...
def resource_path(relative_path):
    """
    ฟังก์ชันนักหาแผนที่: หาตำแหน่งที่แท้จริงของไฟล์ (เช่น config.json)
    ไม่ว่าโปรแกรมจะถูกรันแบบปกติ หรือถูกแปลงเป็นไฟล์ .exe แล้วก็ตาม
    """
    try: # ลองทำตามนี้ก่อน
        # ถ้าโปรแกรมถูกแปลงเป็น .exe มันจะสร้างที่อยู่ชั่วคราวไว้ในตัวแปร _MEIPASS
        base_path = sys._MEIPASS
    except Exception: # แต่ถ้าเกิดข้อผิดพลาด (แสดงว่ารันแบบปกติ)
        # ให้ใช้ที่อยู่ปัจจุบันของไฟล์ .py นี้แทน
        base_path = os.path.abspath(".")
    return os.path.join(base_path, relative_path) # คืนค่าเป็นที่อยู่เต็มๆ ของไฟล์ที่ต้องการ

def load_config():
    """
    ฟังก์ชันพนักงานจัดเตรียม: อ่านไฟล์ config.json เพื่อเอาข้อมูลสำคัญมาเตรียมไว้ให้พร้อมใช้งาน
    """
    config_path = resource_path('config.json')  # ใช้ "นักหาแผนที่" เพื่อหาไฟล์ config.json
...
async def restart_bot():
    ...
    เพื่อป้องกันปัญหาหน่วยความจำรั่ว (Memory Leak) ที่อาจเกิดขึ้นเมื่อโปรแกรมทำงานนานๆ
    """
    await asyncio.sleep(48 * 3600) # หยุดรอเป็นเวลา 48 ชั่วโมง (48 ชั่วโมง * 3600 วินาที)
    logger.info("กำลังรีสตาร์ทบอทตามกำหนดเวลา (48 ชั่วโมง)...") # บันทึก Log ว่ากำลังจะรีสตาร์ท
    try: # ลองรีสตาร์ทด้วยวิธีมาตรฐาน
        os.execv(sys.executable, ['python'] + sys.argv)
    except Exception: # หากวิธีแรกไม่สำเร็จ
        subprocess.Popen([sys.executable] + sys.argv) # ลองอีกวิธีหนึ่ง
        sys.exit() # ปิดโปรแกรมเก่าทันที
